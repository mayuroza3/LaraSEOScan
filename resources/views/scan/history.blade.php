                ...
                <tbody>
                    @foreach ($scans as $index => $scan)
                        <tr>
                            ...
                            <td>
                                <span
                                    class="badge bg-{{ $scan->status === 'COMPLETED' ? 'success' : ($scan->status === 'PENDING' ? 'warning' : 'danger') }}">
                                    {{ ucfirst($scan->status) }}
                                </span>
                            </td>
                            ...
                            <td>
                                ...
                                <a href="{{ route('scan.export.pdf', $scan->id) }}" class="btn btn-danger">📄 Export PDF</a>
                                <a href="{{ route('scan.export.csv', $scan->id) }}" class="btn btn-success">📁 Export
                                    CSV</a>
